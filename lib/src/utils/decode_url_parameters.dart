...
Future<ClientAuth> decodeUrlParameters(
  ...
  uri.queryParameters.forEach((String queryParam, String value) {
    ...
    switch (queryParam) {
      ...
      case ConnectionStringOptions.authMechanism:
        ...
        if (options.authenticationMechanism == AuthenticationScheme.X509) {
          localAuthDbName = r'$external';
        }
        break;
  ...
  if (uri.userInfo.isNotEmpty) {
    ...
    clientAuth.userName = Uri.decodeComponent(userInfo[0]);
    clientAuth.password = Uri.decodeComponent(userInfo[1]);
  } else {
    clientAuth.userName = options.auth?.username;
    clientAuth.password = options.auth?.password;
  }
