...
const dbName = 'mongo-dart-replica';
const dbServerName = 'replica';
...
void main() async {
  ...
  group('Replica Set Test', () {
    tearDownAll(() async {
      await cleanupDatabase();
    });

    group('Server Description', () {
      if (!testInfo.serverfound || !testInfo.isReplicaSet) {
        return;
      }
      test('Simple', () async {
        var db = client.db();
        var collection = db.collection('replica-test');
