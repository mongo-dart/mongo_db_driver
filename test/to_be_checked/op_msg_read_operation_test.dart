...
void main() async {
  ...
  group('Read Operations', () {
    ...
    test('Simple read - extract sub-document', () async {
      ...
      resultOne = await collection.findOne(filter: {
        'pontosGastronomicos': {
          r'$elemMatch': {'id': '208a3f93-9fcb-4db7-ac44-bb11b86a2d31'}
        }
      }, projection: {
    ...
    test('Simple read - using stream from cursor', () async {
      ...
      var result = [];
      var cursor = Cursor(FindOperation(collection, QueryUnion({})), db.server);
    ...
    group('Tailable Cursor', () {
      test('Simple read from capped collection', () async {
        ...
        FindResult fr;
        MongoDocument doc;
        (fr, doc) = await FindOperation(collection, QueryUnion({}),
                findOptions: FindOptions(tailable: true))
            .executeDocument();

        var cursor = Cursor.fromOpenId(
            collection,
            (doc[keyCursor] as Map)[keyId] as Int64,
            db.server,
            fr.cursorResult.session,
            tailable: true);
        ...
        expect(cursor.state, CursorState.closed);
      });
  ...
  group('Aggregate', () {
    ...
    test('Simple Aggregate', () async {
      ...
      var pipeline = AggregationPipelineBuilder()
        ..addStage($group(id: 'games', fields: {
          'total': $sum(Field('cost')),
          'avg': $avg(Field('cost'))
        }));
 ...
 */
  ...
  group('Change Stream', () {
      ...
      var pipeline =
        ...
        <String, Object>{
          ...
          AggregationPipelineBuilder()
            ..addStage($match((where..$lt('fullDocument.a', 15)).rawFilter));
    ...
    test('Change with match from collection.watch()', () async {
      ...
      var aResult = 3;
      var controller = stream.listen((changeEvent) {
        Map<String, dynamic> fullDocument = changeEvent.fullDocument ?? {};
  ...
  group('Count', () {
    ...
    group('Command', () {
      ...
      test('Selected documents - Class result', () async {
        ...
        var operation = CountOperation(collection,
            query: (where..$gt('cost', Decimal.fromInt(15))).rawFilter);
  ...
  group('Distinct', () {
    ...
    group('Collection helper', () {
      test('Return Distinct Values for a Field - Map result', () async {
        ...
        var result = await collection.distinctMap('dept');
      ...
      test('Return Distinct Values for a Field - Class result', () async {
        ...
        var result = await collection.distinct('dept');
      ...
      test('Return Distinct Values for an Embedded Field - Class result',
        ...
        var result = await collection.distinct('item.sku');
      ...
      test(
        ...
        var result =
            await collection.distinct('item.sku', query: {'dept': 'A'});
