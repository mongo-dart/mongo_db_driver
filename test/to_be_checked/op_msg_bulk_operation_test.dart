  ...
  group('Bulk Operations', () {
    ...
    group('Utils', () {
      test('SplitInputOrigins', () {
        var saveMaxDocs = MongoMessage.maxWriteBatchSize;
        MongoMessage.maxWriteBatchSize = 5;
        var originMap = {0: 0, 3: 1, 7: 2, 12: 3};
        var collectionName = getRandomCollectionName();
        var collection = db.collection(collectionName);
        var bulk = OrderedBulk(collection);
        var ret = bulk.splitInputOrigins(originMap, 22);
        MongoMessage.maxWriteBatchSize = saveMaxDocs;
    ...
    group('Bulk insert', () {
      test('Ordered Bulk', () async {
        ...
        var (ret, _) = await bulk.executeDocument(db.server);
    ...
    group('Bulk update', () {
      test('Unordered Bulk', () async {
        ...
        bulk.updateMany(UpdateManyStatement(
            QueryUnion(where..$eq('status', 'D')),
            UpdateUnion(UpdateExpression()..$set('status', 'd'))));

        bulk.updateOne(UpdateOneStatement(
            QueryUnion({'cust_num': 99999, 'item': 'abc123', 'status': 'A'}),
            UpdateUnion(UpdateExpression()..$inc('ordered', 1))));
      ...
      test('Ordered Bulk - collection helper', () async {
        ...
        bulk.updateMany(UpdateManyStatement(
            ...
            UpdateExpression()..$set('status', 'd').map));
